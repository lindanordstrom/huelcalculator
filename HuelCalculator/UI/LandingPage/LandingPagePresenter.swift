...
class LandingPagePresenter {
    ...
    func getTitleAndImageFrom(indexPath: IndexPath) -> (title: String?, image: UIImage?, highlightedImage: UIImage?) {
        ...
        case .huelHotAndSavoury:
            image = #imageLiteral(resourceName: "menu_granola")
            highlightedImage = #imageLiteral(resourceName: "menu_granola_highlighted")
        case .bar:
    ...
    func didSelectItemOnLandingPage(indexPath: IndexPath) {
        ...
        case .huelHotAndSavoury:
            view?.showCalculationPage(with: HuelHotAndSavoury())
        case .bar:
